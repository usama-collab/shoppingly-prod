...
# generic set password form is used when password reset and user receive instructions and click link then see that form is password set form
...
class LoginForm(AuthenticationForm):
    ...
    password = forms.CharField(
        label=("Password"), strip=False, widget=forms.PasswordInput(attrs={'autocomplete': 'current-password', 'class': 'form-control'}))
...
class MyPasswordChangeForm(PasswordChangeForm):
    ...
    new_password1 = forms.CharField(
        label=_('New Password'), strip=False, widget=forms.PasswordInput(attrs={'autocomplete': 'new-password',
                                                                                'class': 'form-control'}),
...
class CustomerProfileForm(forms.ModelForm):
    class Meta:
        model = Customer
        fields = ['name', 'locality', 'city', 'state', 'zipcode', 'state']
        widgets = {
